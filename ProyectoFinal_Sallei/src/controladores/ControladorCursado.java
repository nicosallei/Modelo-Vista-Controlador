...
public class ControladorCursado implements ActionListener {
    ...
    public void agregar() {
        ...
        if (revisarCampos() == false) {
            ...
            cursado.getAlumnoDni().setDni(splitearString(agregarCursado.getCbxAlumnos().getSelectedItem().toString()));
    ...
    public void editar() {
        if (val.validarNota(Double.parseDouble(editarCursado.getTxtNota().getText())) == false) {
            ...
            cursado.setAlumnoDni(new Alumno());
            cursado.getAlumnoDni().setDni(Integer.parseInt(editarCursado.getTxtAlumno().getText()));
    ...
    public void listarCursados(JTable table) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            fila[0] = lista.get(i).getAlumnoDni().getDni();
